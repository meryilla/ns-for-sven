...
const float TURRET_TRACKING_RATE = 2.0f;
...
mixin class TurretBase
{
    ...
    void SetEnabledState()
    {
        ...
        if( iEnabledAnimation >= 0 )
        {
            float flSpeed = m_blIsBuilt ? 1.0f : -1.0f;
            PlayAnimationAtIndex( iEnabledAnimation, true, flSpeed );
            //self.pev.sequence = iEnabledAnimation;
        }
        if( !m_blIsBuilt )
        {
            self.m_hEnemy = null;
